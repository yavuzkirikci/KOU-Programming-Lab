...
public class SarkiDuzenle {
    ...
    public void sarkilariGoster(){
        try {
            sarkiMenu.getItems().clear();
            albumIDBul();
            Main.st = Main.conn.createStatement();
            String sorgu = String.format("select * from sarkilar where sanatciID = %d and albumID = %d order by sarkiAdi ASC", sanatciID, albumID);
            ResultSet rs = Main.st.executeQuery(sorgu);
            while (rs.next()){
                String ad = rs.getString("sarkiAdi");
                MenuItem menuItem = new MenuItem(ad);
                menuItem.setOnAction(new EventHandler<ActionEvent>() {
                    @Override
                    public void handle(ActionEvent actionEvent) {
                        secilenSarki = menuItem.getText();
                        sarkiMenu.setText(secilenSarki);
                        sarkiIDBul();
                    }
                });
                sarkiMenu.getItems().add(menuItem);
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public void sarkiIDBul(){
        try {
            Main.st = Main.conn.createStatement();
            String sorgu = String.format("select * from sarkilar where sarkiAdi = '%s'", secilenSarki);
            ResultSet rs = Main.st.executeQuery(sorgu);
            if (rs.next()){
                sarkiID = rs.getInt("sarkiId");
                Main.duzenlenecekSarkiID = sarkiID;
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public void duzenleClicked(MouseEvent mouseEvent) {
        try {
            AnchorPane pane = FXMLLoader.load(getClass().getResource("sarkiDuzenle2.fxml"));
            anchor.getChildren().setAll(pane);
        } catch (IOException e) {
            e.printStackTrace();
        }
