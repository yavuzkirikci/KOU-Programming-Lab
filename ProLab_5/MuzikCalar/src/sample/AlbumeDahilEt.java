...
public class AlbumeDahilEt {
    ...
    public void dahilEt(MouseEvent mouseEvent) throws SQLException {
        if (!albumVarMi()){
            System.out.println("Sistemde girdiğiniz isimde bir albüm bulunmamakta");
        }
        else if (!sarkiciyiKontrolEt()){
            System.out.println("Sistemde girdiğiniz isimde bir sanatçı bulunmamakta");
        }
        else if (sanatciAlbumdeVarMi()){
            System.out.println("Sanatçı zaten bu albümde bulunmakta");
        }
        else{
            Main.st = Main.conn.createStatement();
